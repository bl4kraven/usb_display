...
// for USB touch protocol
static usb_touch_callback g_fun;
static void * g_data;
        ...
        struct pixcir_report_data *report)
{
    ...
    //dev_err(&tsdata->client->dev, "readsize %d, max_fingers %d", readsize, tsdata->max_fingers);
    ...
    touch = rdbuf[0] & 0x7;
    //dev_err(&tsdata->client->dev, "touch %d", touch);
...
static struct pixcir_ts_platform_data *pixcir_parse_dt(struct device *dev)
{
    ...
    pdata->chip.max_fingers = 5;
    pdata->chip.has_hw_ids = true;
        ...
        const struct i2c_device_id *id)
{
    ...
    // TODO
    // 发现GPIO已经被申请了，所以Hack一下，这个临时方案有风险
    //gpio_free(pdata->gpio_attb);
    ...
    error = devm_gpio_request_one(dev, pdata->gpio_reset,
            GPIOF_OUT_INIT_LOW, "pixcir_i2c_reset");
    if (error) {
...
int pixcir_init(usb_touch_callback fun, void *data)
{
    g_fun = fun;
    g_data = data;
